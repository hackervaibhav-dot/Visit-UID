...
GITHUB_REPO = "hackervaibhav-dot/Visit-UID"   # <-- change
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN")
UPDATE_BRANCH = "main"

def github_upsert_file(path_in_repo: str, content_text: str, message: str) -> bool:
    if not GITHUB_TOKEN:
        print("⚠️ No GITHUB_TOKEN - skipping GitHub update")
        return False
    ...
    # 1) Get existing SHA (if file already exists)
    sha = None
    r = httpx.get(api_url, headers=headers, params={"ref": UPDATE_BRANCH}, timeout=15.0)
    ...
    r2 = httpx.put(api_url, headers=headers, json=payload, timeout=20.0)
    if r2.status_code in (200, 201):
        print("✅ GitHub file updated:", path_in_repo)
        return True

    print("❌ GitHub PUT error:", r2.status_code, r2.text)
    return False


def save_token_cache(server, tokens):
    """SAVE TOKENS LOCALLY (/tmp) + OPTIONAL GitHub UPDATE"""
    cache_file = f"/tmp/token_cache_{server.lower()}.json"  # Vercel writable
    data = {
        "server": server,
        "tokens": len(tokens),
        "timestamp": time.time(),
    ...
    # If you want: push sometimes (10% chance)
    if random.random() < 0.1:
        github_upsert_file(
            path_in_repo=f"token_cache_{server.lower()}.json",
            content_text=json.dumps(data, indent=2),
            message=f"Auto token update {time.strftime('%Y-%m-%d %H:%M')}"
        )
